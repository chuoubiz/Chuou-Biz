...
const CategoryPage = ({ data, pageContext }) => {
  ...
  const { category, currentPage, numPages, limit, skip } = pageContext;
  const posts = category.categoryId === 'all' ? allMicrocmsPosts.edges : allMicrocmsPosts.edges.slice(skip, skip + limit);
              ...
              <div className="column_box" key={node.postsId}>
                ...
                <div className="column_txt">
                  ...
                  {stripHTML(node.content).length > MAX_CONTENT_LENGTH
                    ? stripHTML(node.content).substring(0, MAX_CONTENT_LENGTH) + '...' // 最大文字数を超える場合は、文字列を切り詰めます
            ...
            {numPages > 1 && (
              <div>
                {Array.from({ length: numPages }, (_, i) => (
                  <Link key={`pagination-link${i + 1}`} to={i === 0 ? `/${category.categoryId}/` : `/${category.categoryId}/${i + 1}/`}>
                    {i + 1}
                  </Link>
                ))}
              </div>
