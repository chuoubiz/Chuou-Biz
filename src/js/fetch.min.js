...
function fetch_html(url,id){fetch(url,{method:'GET',mode:'cors',credentials:'include'}).then(function(response){if(response.ok){return response.text()}else{throw new Error()}}).then(function(text){document.getElementById(id).innerHTML=text}).catch(function(error){console.log(error)})}